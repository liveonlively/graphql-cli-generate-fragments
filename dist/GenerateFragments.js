...
var GenerateFragments = /** @class */ (function () {
    ...
    GenerateFragments.prototype.processFragments = function (schemaPath) {
        ...
        if (fragments.length !== 0 && fragments[0].name !== "*") {
            var basePath_1 = outputPath.split('/').slice(0, -1).join('/');
            fragments.map(function (_a) {
                var name = _a.name, fragment = _a.fragment;
                fs.writeFileSync(basePath_1 + "/" + name + "." + extension, fragment, { flag: "w" });
            });
    ...
    GenerateFragments.prototype.makeFragments = function (schemaContents, generator, modelConfigs) {
        ...
        var modelNames = Object.keys(modelConfigs);
        var deepFragments = flatten(typeNames.map(function (typeName) {
            ...
            if (modelConfigs && modelConfigs[name]) {
                return Object.keys(modelConfigs[name]).map(function (suffix) {
                    ...
                    fragment = "fragment " + name + suffix + " on " + name + " {\n  " + spreadFragments.join(_this.indentedLine(1)) + fields.filter(function (f) { return allowlist.indexOf(f.split(' ')[0]) !== -1; }).join(_this.indentedLine(1)) + "\n}";
