...
const modelConfigs: { [k: string]: ModelConfig } = JSON.parse(fs.readFileSync('../../server/model_configs.json', 'utf-8'))
...
export class GenerateFragments {
  ...
  private processFragments(
    ...
    const fragments: Array<Fragment> = this.makeFragments(schemaContents, extension);
  ...
  private makeFragments(schemaContents: string, generator: string): Array<Fragment> {
...
`
  ...
  ${spreadFragments.join(this.indentedLine(1))}${fields.filter((f: string) => allowlist.indexOf(f) !== -1).join(this.indentedLine(1))}
}
`,
          };
        });
