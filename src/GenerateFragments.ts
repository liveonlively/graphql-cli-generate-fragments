...
type ModelConfig = {
  [k?: ModelFragmentType]: string[];
}
const modelConfigs: { [k: string]: ModelConfig } = JSON.parse(fs.readFileSync('../../server/model_configs.json'))
const filterFieldList = (name: string, type: ModelFragmentType, fields: string[]): string[] => {
  if (!(name in modelConfigs)) {
    return [];
  }
  const modelConfig = modelConfigs[name];
  const allow = modelConfig[type];
  return fields.filter((field) => allow.indexOf(field) === -1);
}
...
export class GenerateFragments {
  ...
  private processFragments(
    ...
    const fragments: { name: string; fragment: string } = this.makeFragments(schemaContents, extension);

    if (true || generator === "multi-file-graphql") {
      fragments.map(({ name, fragment }) => {
        fs.writeFileSync(outputPath.replace(`${this.projectName}.fragments.${extension}`, `${name}.graphql`), fragment, { flag: "w" });
      });
    }
  ...
  private makeFragments(schemaContents: string, generator: string): Array<{ name: string; fragment: string }> {
...
`
    ...
    const deepFragments = typeNames.flatMap(typeName => {
      const type: any = ast.getType(typeName);
      const { name } = type;
  ...
  ${spreadFragments.join(this.indentedLine(1))}${fields.filter((f: string) => allowlist.indexOf(field) !== -1).join(this.indentedLine(1))}
}
`,
          }];
        }).filter((x) => !!x);
